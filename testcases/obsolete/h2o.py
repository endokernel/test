...
for i in range(0, len(variable.paths)):
    ...
    if variable.paths[i] == 'baseline':
        filesuffix = "nginx_https_baseline"
        servercmd = "LD_LIBRARY_PATH=../libs/nocet ../bin/nocet/nginx -c ../conf/nginx_https.conf -p ../www"
        curbench = "nginx_https Beseline"
    else:
        filesuffix = "nginx_https_" + variable.paths[i].split("/")[-2]
        servercmd = variable.paths[i] + "src/libintravirt/libintravirt.so " + variable.glibcpath + " ../bin/nocet/nginx -c ../conf/nginx_https.conf -p ../www"
        curbench = "nginx_https " + variable.paths[i].split("/")[-2]
...
for i in range(0, len(variable.cet_path)):
    # launch nginx
    filesuffix = "nginx_https_" + variable.cet_path[i].split("/")[-2]
    servercmd = variable.cet_path[i] + "src/libintravirt/libintravirt.so " + variable.cet_glibcpath + " ../bin/cet/nginx -c ../conf/nginx_https.conf -p ../www"
...
print("Nginx_https total time: " + printable_time)
os.system("echo \"Nginx_https: " + printable_time +"\" >> ../" + variable.resdir + "/time.txt")
