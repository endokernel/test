...
for i in range(0, len(variable.paths)):
    ...
    if variable.paths[i] == 'baseline':
        ...
        filesuffix = "nginx_https_safebox_" + variable.paths[i].split("/")[-2]
        servercmd = variable.paths[i] + "src/libintravirt/libintravirt.so /home/bi1/src/intravirt/glibc-safebox/glibc-nocet/install/lib ../bin/nocet/nginx -c ../conf/nginx_https.conf -p ../www"
        curbench = "nginx_https_safebox " + variable.paths[i].split("/")[-2]
...
for i in range(0, len(variable.cet_path)):
    # launch nginx
    filesuffix = "nginx_https_safebox_" + variable.cet_path[i].split("/")[-2]
    servercmd = variable.cet_path[i] + "src/libintravirt/libintravirt.so /home/bi1/src/intravirt/glibc-safebox/glibc-cet/install/lib ../bin/cet/nginx -c ../conf/nginx_https.conf -p ../www"
...
os.system("echo \"Nginx_safebox: " + printable_time +"\" >> ../" + variable.resdir + "/time.txt")
